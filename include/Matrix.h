...
/** @class Matrix
 *  @brief Two-dimensional templatized data structure. (y, x) (Rows, Columns).
 *
...
class Matrix
{
    public:
         ...
         */
        bool loadFromFile(std::string filename, int rows);
         ...
         */
        void addRow(std::vector<T> new_row);
         ...
         *  @param new_row The vector of type T to add to the Matrix.
         */
        void addColumn(std::vector<T> new_column);
        ...
        const std::vector<T> & getRow(unsigned int r)const;
        std::vector<T> & getRow(unsigned int r);
        std::vector<T> getRow(unsigned int r, std::vector<int> columns)const; 
        std::vector<T> getColumn(unsigned int c)const;
        const T & a(int y, int x)const;
        T & a(int y, int x);
        
        bool replaceRow(int r, const std::vector<T> & newRow);
        ...
        Matrix orderRows(std::vector<int> v)const;
        ...
        //std::vector<int> sortByColumn(int column, bool asNumeric, bool ascending);
        
        bool sortHelper(const std::vector<T> & v1, const std::vector<T> & v2/*std::vector<T>::iterator it1, std::vector<T>::iterator it2*/);
        void sortt(int column);

        /*printing*/
        void print()const;
        void printToFile(std::ofstream & ofs, std::string delimiter)const;

    private:
        std::vector<std::vector<T> > d_; /**< Underlying vector of vectors storage structure for type T. */
...
Matrix<T>::Matrix()
{
    d_ = std::vector<std::vector<T> >(0, std::vector<T>(0, T()));
    rows_=0;
    columns_=0;
}
...
{
    d_ = std::vector<std::vector<T> >(rows_in, std::vector<T>());

    //#pragma omp parallel for
    for(unsigned int i=0; i<rows_in; i++)
        d_[i] = std::vector<T>(columns_in, T());
...
{
    d_ = std::vector<std::vector<T> >(r.rows_, std::vector<T>());

    //#pragma omp parallel for
    for(unsigned int i=0; i<r.rows_; i++)
        d_[i] = std::vector<T>(r.columns_, T());
...
template<class T>
Matrix<T>::Matrix(std::string num_string)
{
    std::stringstream ss;
    ss<<num_string;
...
void Matrix<T>::clear()
{
    ...
    columnLabels_ = std::vector<std::string>();
}
...
/*begin file loading-----------------------------------------------*/
template<class T>
bool Matrix<T>::loadFromFile(std::string filename, int rows)
{
...
{
    d_.push_back(std::vector<T>(columns_));
    ++rows_;
}
...
template<class T>
void  Matrix<T>::addRow(std::vector<T> new_row)
{
    if(empty())
    {
        ...
        d_.push_back(std::vector<T>(new_row.size(), T()));
    ...
    {
        std::cout<<"NEW ROW DIMENSION MISMATCH "<<rows_<<" SIZE "<<new_row.size()<<" TO "<<dim(1)<<std::endl;
        return;
    }
    else
    {
...
template<class T>
void  Matrix<T>::addColumn(std::vector<T> new_column)
{
    if(empty())
    {
        ...
        {
            d_.push_back(std::vector<T>(1,T()));
            d_[i][0] = new_column[i];
        }
    }
    else if(new_column.size() != rows_)
    {
        std::cout<<"NEW COLUMN DIMENSION MISMATCH"<<std::endl;
        return;
    }
    else
    {
...
template<class T>
void Matrix<T>::setColumnLabels(std::vector<std::string> V)
{
    columnLabels_ = V;   
}

template<class T>
const std::vector<std::string> & Matrix<T>::getColumnLabels()const
{
...
Matrix<T> Matrix<T>::get_sub(unsigned int y1, unsigned int y2, unsigned int x1, unsigned int x2)const
{
    ...
    {
        std::cout<<"SUB_MATRIX DIMENSIONS INVALID"<<std::endl;
        return Matrix<T>();
    }
...
template<class T>
const std::vector<T> & Matrix<T>::getRow(unsigned int r)const
{
    return d_.at(r);
}

template<class T>
std::vector<T> & Matrix<T>::getRow(unsigned int r)
{
    return d_[r];
}

template<class T>
std::vector<T> Matrix<T>::getRow(unsigned int r, std::vector<int> columns)const
{
    std::vector<T> retVal;
...
template<class T>
std::vector<T> Matrix<T>::getColumn(unsigned int c)const
{
    std::vector<T> n_m(rows_, T());
...
template<class T>
bool Matrix<T>::replaceRow(int r, const std::vector<T> & newRow)
{
    if(newRow.size() != dim(1))
    {
        std::cout<<"ROW SIZE MISMATCH "<<dim(1)<<" "<<newRow.size()<<std::endl;
        return false;
    }
...
template<class T>
Matrix<T> Matrix<T>::orderRows(std::vector<int> v)const
{
    Matrix<T> retVal;
...
template<class T>
void Matrix<T>::printToFile(std::ofstream & ofs , std::string delimiter)const
{
    for(int x=0; x<d_.size(); x++)
    {
        for(int y=0; y<d_[0].size(); y++)
