...
{   
     ...
     */
    template<class T>
    static Matrix<T> makeRowsUniqueOnColumns(const Matrix<T> & m, std::vector<int> c);
    
    //REFORMULATE TO WORK WITH VECTORS OF FUNCTION POINTERS
    template<class T>
    static Matrix<T> removeRowsWith(const Matrix<T> & m, const std::vector<int> & c, const std::vector<T> & v, bool (*foo1)(const bool, const bool), bool (*foo2)(const T &, const T &));
    ...
    template<class T>
    static std::pair<Matrix<T>, Matrix<T> > permuteColumns(const Matrix<T> & m1, const Matrix<T> & m2 );
...
/*-----------------------Implementation-----------------------------*/
    ...
    Matrix<T> elementWiseOperation(const Matrix<T> & m1, const Matrix<T> & m2, T (*foo)(const T &, const T &))
    {
        ...
        {
            std::cout<<std::endl<<"DIMENSION MISMATCH"<<std::endl;
            return Matrix<T>();
        }
    }
    template<class T>
    ...
    Matrix<T> MatrixProduct(const Matrix<T> & m1, const Matrix<T> & m2)
    {
        ...
        {
            std::cout<<std::endl<<"INNER DIMENSION INNEQUALITY"<<std::endl;
            return Matrix<T>();
        }
    ...
    template<class T>
    Matrix<T> makeRowsUniqueOnColumns(const Matrix<T> & m, std::vector<int> c)
    {
        Matrix<T> retVal;
        
        std::map<std::vector<T>, char> uniqueChecker;
        
        for(int i=0; i<m.dim(0); ++i)
        {
            
            //Generate Key
            std::vector<T> tempKey;
    ...
    template<class T>
    Matrix<T> removeRowsWith(const Matrix<T> & m, const std::vector<int> & c, const std::vector<T> & v, bool (*foo1)(const bool, const bool), bool (*foo2)(const T &, const T &))
    {
        if(c.size() != v.size())
        {
            std::cout<<"Remove Row Vector Size Missmatch"<<std::endl;
            return Matrix<T>();
        }
    ...
    static std::pair<Matrix<T>, Matrix<T> > permuteColumns(const Matrix<T> & m1, const Matrix<T> & m2 )
    {
        ...
        std::vector<int> ordering(m1.dim(1) + m2.dim(1),0);
        ...
        std::pair<Matrix<T>, Matrix<T> > retVal;
        ...
        std::cout<<retVal.first.dim(0)<<" "<<retVal.first.dim(1)<<" "<<retVal.second.dim(0)<<" "<<retVal.second.dim(1)<<std::endl;
        return retVal;
    }
}
