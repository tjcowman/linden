...
namespace Linden::Core
{
    ...
    LDForest::LDForest(Genetics::Id::Snp numSnps) :
        topSnpList_(TopSnpList(numSnps)){
    }

    LDForest::LDForest(SnpSet& snpSet, Genetics::Id::Snp numSnps) :
        topSnpList_(TopSnpList(numSnps)) {
        ...
        for (Genetics::Id::Snp i = 0; i < s.size(); ++i)
        {
    ...
    size_t LDForest::mergeTreeIteration(float unknownFraction){
        ...
        Genetics::Id::Snp allowedDifferences = unknownFraction * (Snp::getDimensions().numControls_ + Snp::getDimensions().numCases_); //(log_->controls_ + log_->cases_);
    ...
    void LDForest::to_serial(std::ostream& os, const LDForest& e) {
        ...
        Genetics::Id::Snp numSnps = e.topSnpList_.size();
        os.write(reinterpret_cast<const char*>(&numSnps), sizeof(Genetics::Id::Snp));
        ...
        Genetics::Id::Snp numTrees = e.ldtrees_.size();
        os.write(reinterpret_cast<const char*>(&numTrees), sizeof(Genetics::Id::Snp));
    ...
    {
        Genetics::Id::Snp numSnps;
        Snp::Dimensions dim;
        Genetics::Id::Snp numTrees;

        is.read(reinterpret_cast<char*>(&numSnps), sizeof(Genetics::Id::Snp));
        is.read(reinterpret_cast<char*>(&dim), sizeof(Snp::Dimensions));
        is.read(reinterpret_cast<char*>(&numTrees), sizeof(Genetics::Id::Snp));

        Snp::setDimensions(dim.numControls_, dim.numCases_);
        LDForest e(numSnps);

        e.ldtrees_.reserve(numTrees);
        for (Genetics::Id::Snp i = 0; i < numTrees; ++i)
